...
export const $$:A.createStatesSerializer = ($d) => {
    return <Annotation>($: g_algorithm.T.States<Annotation>, $i: g_fp.SYNC.I.Block) => {
        function serializeType($: g_algorithm.T.Type<Annotation>, $i: g_fp.SYNC.I.Line) {
            switch ($[0]) {
                case 'null':
                    pl.cc($[1], ($) => {
                        $i.snippet(`null`)
